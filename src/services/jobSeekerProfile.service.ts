...
export const saveJobSeekerProfile = async (jobSeekerParams: Params) => {
  try {
    ...
    jobSeekerProfileRepository.save(jobSeekerParams);
...
export const getJobSeekerProfileForServerless = async (jobKeySkills: number[]) => {
  try {
    const jobSeekerProfileRepository = AppDataSource.getRepository(JobSeekerProfile);
    const jobSeekerProfile = await jobSeekerProfileRepository.find({
      where: {
      ...
      relations: {
        ...
        keySkills: {
          profileKeySkills: true
        },
        currentCompany: true,
        currentJobTitle: true,
        educations: true,
        careerProfile: {
          careerProfilePreferredLocations: {
            location: true
          }
        },
        user: true,
    ...
    return jobSeekerProfile.slice(0, 3);
  } catch (error) {
    console.log('error', error);
    throw error;
  }
