  ...
  try {
    ...
    if (jobsParams?.id) {
      ...
      jobs.title = jobsParams?.title ? jobsParams?.title : jobs.title,
        ...
        jobs.employmentType = jobsParams?.employmentType ? jobsParams?.employmentType : jobs.employmentType,
        ...
        jobs.isDraft = jobsParams?.isDraft,
...
export const userBasedJobsList = async (id: number) => {
  try {
    const jobsRepository = AppDataSource.getRepository(Jobs);
    let jobs = await jobsRepository.find({
      where: {
