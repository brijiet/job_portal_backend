...
import { registerUser, resetPassword, signInUser, updatePassword } from '../controllers/user.controller';
import { Request } from 'express-jwt';
import { SignOut } from '../controllers/signOut.controller';
import verifyUser from '../middlewares/isAuthentcated';
const reactServer = process.env.REACT_SERVER_URL as string;
...
authRouter.get('/google/callback',
  passport.authenticate('google',
    {
      failureRedirect: `${reactServer}registration`, session: true
    }),
  jwtSign
)
authRouter.get('/logout', (req: Request, res: Response, next) => {
  res.cookie('token', null);
  res.redirect(reactServer)
});
...
authRouter.put('/updatePassword', passport.authenticate('jwt'), updatePassword);
//routers for O2Auth
